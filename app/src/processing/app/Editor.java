...
*/
    ...
    item.addActionListener(event -> Base.openURL(tr("http://www.energia.nu/")));
    menu.add(item);
...
//    toolbar.deactivate(EditorToolbar.STOP);
  ...
  public void updateTitle() {
    ...
    if (customFormat != null && !customFormat.trim().isEmpty()) {
      ...
      titleMap.put("version", BaseNoGui.VERSION_NAME_LONG + "E" + BaseNoGui.EVERSION_NAME_LONG);

      setTitle(StringReplacer.replaceFromMapping(customFormat, titleMap));
    } else {
      if (current.isPrimary()) {
        setTitle(I18n.format(tr("{0} | Energia {1}"), sketch.getName(),
                             BaseNoGui.VERSION_NAME_LONG + "E" + BaseNoGui.EVERSION_NAME_LONG));
      } else {
        setTitle(I18n.format(tr("{0} - {1} | Energia {2}"), sketch.getName(),
                             current.getFileName(), BaseNoGui.VERSION_NAME_LONG + "E" + BaseNoGui.EVERSION_NAME_LONG));
      }
    }
  }
